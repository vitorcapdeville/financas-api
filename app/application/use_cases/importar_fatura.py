"""
Use Case: Importar Fatura de Cartão
"""
...
class ImportarFaturaUseCase:
    """
    Caso de uso para importar transações de fatura de cartão de crédito.
    
    Regras de negócio:
    - Arquivo deve ser CSV ou Excel
    - Colunas obrigatórias: data, descricao, valor
    - Todas as transações são do tipo "saída"
    - Valores sempre positivos (absolutos)
    - Tag "Rotina" é adicionada automaticamente
    - Suporta coluna opcional data_fatura
    ...
    def execute(
        ...
        """
        Importa transações de fatura de cartão.
        ...
        for _, row in df.iterrows():
            try:
                ...
                # Converter valor (sempre positivo para fatura)
                ...
                if 'data_fatura' in row and pd.notna(row['data_fatura']):
                    data_fatura = self._converter_data(row['data_fatura'])
                ...
                transacao = Transacao(
                    ...
                    tipo=TipoTransacao.SAIDA,  # Fatura sempre é saída
                    categoria=categoria,
